...
**Status:** **M1–M5 frontend complete** for AI Search: assistant-ui runtime (`Tools` Toolkit + `useAui`), tool-ui-pattern cards (`tool-cards.tsx` / `tool-card-view.tsx`), localStorage thread persistence, sidebar thread list, auto titles, effort control, stop/cancel, error banner. Backend: multi-provider SSE agent, MCP registry. **Remaining (ops scale):** Redis semantic MCP tool selection (`mcp/vector.ts`), Postgres server-side thread sync, full npm tool-ui gallery vendoring (demo has extended widgets under `docs/demos/assistant-ui-hd-demo/src/components/tool-ui/`).
...
The LLM runs the query with the selected tools (in the user-configured order), streams back **text + Tool-UI output**. HD-Search is free and self-hostable — there is **no credit metering, quota, or billing**; runs are unlimited. Token usage is still recorded for telemetry (it feeds the model auto-selector and dashboards), but nothing is charged.
...
- Response = **text + tool UI**, streamed, with citations and per-run usage/latency telemetry (no cost/credit accounting — usage is unlimited and free).
...
```
        ...
        (vec + cache)    (usage stats)  endpoints     (tools)
```

New code (all under `api/src/`):
- `routes/ai.ts` — `/v1/ai/*` endpoints (local-auth gate; no quota/credit gate — usage is unlimited).
...
2. **Model selection** (`model-selector.ts`):
   - If the user set **auto-select** → run the **OR optimizer** (§7) over the eligible LLM models (those the user has keys for, that meet the query's requirements) to pick `(provider, model)`.
...
4. **Agent loop** (`ai/orchestrator.ts`): call the chosen LLM with the system prompt + history + tool set; on `tool_use`, execute the tool (built-in or via MCP client), append results, loop until the model returns a final answer (`end_turn`) or hits a **step ceiling** (a safety cap on runaway loops — not a billing limit).
5. **Stream** text + tool-UI blocks to the browser as they arrive (SSE).
6. **Record** the run: sum input/output tokens and latency → write a usage row for telemetry/dashboards (§9). No charge — usage is unlimited and free.
7. **Return** final message with citations and the models/tools used.
...
interface LlmModel {
  ...
  // pricing in USD per 1M tokens — drives the cost objective in the auto-select optimizer
...
- MCP: the Messages API can connect to remote MCP servers directly (`mcp_servers` param), or we drive local/remote MCP via our own client (§8) and pass converted tools. v1 uses **our own MCP client** so tool selection and vault-less per-user creds stay in our control.
...
pick = argmin_m score(m)      subject to capability + key constraints
```
...
- **Hard constraints** (filtered, not scored): `contextTokens ≥ prompt_tokens`, required capabilities present, model `enabled`, user has a key (or it's self-hosted). Estimated cost `c` is a *soft* scoring input (cheaper-is-better), not a hard budget gate — there is no credit budget.
...
## 9. Usage recording & safety limits

> HD-Search is **free and self-hostable — there is no credit metering, quota, billing, or plan gating**. Runs are unlimited. Token/latency usage is still recorded, purely for **telemetry** (it feeds the auto-select optimizer §7/§14 and dashboards) and as a safety valve against runaway agent loops.

### 9.1 Runaway-loop ceiling
- A hard **step ceiling per run** (config, e.g. a max number of tool/model turns) stops runaway agent loops; the orchestrator aborts and returns partial output + the reason. This is a safety cap, **not** a billing limit.

### 9.2 Storage
- `ai_runs` (Timescale hypertable): `ts, user_id, thread_id, model, providers_used[], tools_used[], input_tokens, output_tokens, status, took_ms`. Recorded best-effort (never fails the request); used for dashboards and the optimizer's EWMAs.
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/v1/ai/chat` | **SSE stream.** Body: `{messages[], modelOverride?, autoSelect?, weights?, mcp?, effort?, threadId?}`. Streams text + tool-ui events; ends with usage stats (tokens/latency). |
| GET | `/v1/ai/models` | List LLM models (availability per user keys, capabilities, default rank). |
...
| GET | `/v1/ai/runs` | Usage/telemetry history for AI Mode (Timescale). |

Auth: same `requireAuth` as the rest of the API — local email+password session (roles `admin`/`user` from the DB `role` column). AI Mode is available to every authenticated user; no plan entitlement or credit gate.
...
event: usage        data: {inputTokens, outputTokens, tookMs}   // telemetry only — no cost/credits
...
- **Postgres** (`hd_search` schema): `ai_threads(id, user_id, title, created_at)`, `ai_messages(thread_id, role, content, tool_calls, created_at)`, `ai_runs` (§9.2), `ai_prefs(user_id, auto_select, weights, model_rank[], mcp_rank[], pinned_tools[])`, `ai_mcp_servers(id, label, transport, url, requires_keys[], enabled, rank)`.
- **Redis** (`hdsearch-redis`): tool-vector index `ai-tools` (RediSearch HNSW); response cache for identical `(model, prompt, tools)` (short TTL, opt-out); rate-limit buckets.
...
- Per-run **step ceiling** stops runaway loops; per-user rate limits (reuse the limiter) protect the instance. There is no credit/quota gate — usage is unlimited.
...
1. **M1 — Skeleton:** `AI` tab; `/v1/ai/chat` SSE with **Anthropic Opus 4.8 only**, streaming, built-in `hd_search`/`hd_crawl` tools; assistant-ui surface (no MCP yet). Manual model dropdown.
2. **M2 — Usage telemetry:** `ai_runs` recording, dashboard usage/latency charts; per-run step ceiling (runaway-loop safety).
...
## 16. Open decisions
- **Direct Anthropic `mcp_servers`** vs **our own MCP client** for tool execution — v1 uses our client (control over tool selection); revisit if direct connector is materially simpler for read-only servers.
...
function autoSelect(cands: Candidate[], w = { cost: 0.4, latency: 0.3, fail: 0.3 }): LlmModel[] {
  const ok = cands.filter(c => c.eligible);                 // capability + key gates (no budget gate)
